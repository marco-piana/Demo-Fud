...
class FrontEndController extends Controller
{
    ...
    public function restorant($alias)
    {
        ...
        if ($restorant && $restorant->active == 1) {
            ...
            //Brij Negi Update
            // Define $forceLogin dynamically or set a default value
            $forceLogin = 'no';
            $authUser = null;
            $forceLoginDBValue = $restorant->force_login;
            
            if ($forceLoginDBValue  == 1) {
                $forceLogin = 'yes';
                if(session()->has('UserViewer')){
                    $authUserID = session('UserViewer');
                    $authUser = UserViewer::findOrFail($authUserID);
                }
            }
