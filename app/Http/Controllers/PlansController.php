...
class PlansController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $plan->can_pickup = $request->can_pickup == 'true' ? 1 : 0;
        $plan->can_deliver = $request->can_deliver == 'true' ? 1 : 0;
        $plan->free_deliver = $request->free_deliver == 'true' ? 1 : 0;
        $plan->order_table = $request->order_table == 'true' ? 1 : 0;
        $plan->table_reservation = $request->table_reservation == 'true' ? 1 : 0;
    ...
    public function subscribe3dStripe(Request $request, Plans $plan, User $user): RedirectResponse
    {
        if ($request->success . '' == 'true') {
            ...
            if ($restorant && $plan && $user) {
                $restorant->can_pickup = $plan->can_pickup;
                $restorant->can_deliver = $plan->can_deliver;
                $restorant->free_deliver = $plan->free_deliver;
                $restorant->order_table = $plan->order_table;
                $restorant->table_reservation = $plan->table_reservation;
                $restorant->update();
    ...
    public function adminupdate(Request $request): RedirectResponse
    {
        ...
        return redirect()->route('admin.restaurants.edit', $request->restaurant_id)->withStatus(__('Something went wrong'));
    }
