...
<section class='section section-place-content'>
    ...
    <div class='bg-white nav-menu sticky_horizontal'>
        <div class='packer'>
            <div class="package">
                <nav id='place-menu' class="flex drop-cat tabbable content-tab expanded">
                    <ul class="nav nav-tabs flex" role="tablist">
                        ...
                        <li class="dropdown relative" role="presentation">
                            <a aria-expanded="false" role="button" href="#" data-toggle="dropdown" class="nav-link active dropdown-toggle "  data-flip="false">
                                {{ __('Categories') }} <span class="caret"></span>
                            </a>
                        
                            <ul role="menu" class="dropdown-menu" x-placement="left">    
                                <li class="nav-item nav-item-category ">
                                    <a class="nav-link active" data-toggle="tab"  href="">{{ __('All categories') }}</a>
                                </li>
                                @foreach ( $restorant->categories as $key => $category)
                                    @if(!$category->items->isEmpty())
                                        <li class='item nav-item nav-item-category' id="{{ 'cat_'.clean(str_replace(' ', '', strtolower($category->name)).strval($key)) }}">
                                            <a class="nav-link" data-toggle="tab"  id="{{ 'nav_'.clean(str_replace(' ', '', strtolower($category->name)).strval($key)) }}" href="#"><?php echo $category->name ?></a>
                                        </li>
                                    @endif
                                @endforeach
                            </ul>
        ...
        <div class='package'>
            <div class='content'>
                <div class="row">
                    <div class="col-xl-9">
                        <!-- tab menu -->
                        <div class='holder-left content-tab expanded tab-pane' id="{{ ''.clean(str_replace(' ', '', strtolower($category->name)).strval($key)) }}" role="tabpanel"'>
                            <div class='content-center'>
                                @if(!$restorant->categories->isEmpty())
                                    @foreach ( $restorant->categories as $key => $category)

                                    <div id="{{ clean(str_replace(' ', '', strtolower($category->name)).strval($key)) }}" class="box-info {{ ''.clean(str_replace(' ', '', strtolower($category->name)).strval($key)) }} ">
                                        <div class='head'> 
                                        ...
                                        <div class='content grid'>
                                            @foreach ($category->aitems as $item)
                                                ...
                                                <a href='javascript:;' disabled class='item-offer-horizontal disabled'>
                                                    <div class="no-stock">
                                    ...
                                    <div class="row justify-content-center ">
                                        <div class="w-100 ">
                                            <div class="text-center">
                                                ...
                                                <lottie-player src="https://assets1.lottiefiles.com/packages/lf20_mznpnepo.json"  background="transparent"  speed="1"  style="width: 100%; height: auto;"  loop autoplay></lottie-player>
                                            </div>
                    ...
                    <div class="col-xl-3">
                        ...
                        <div class="btn_reserve_fixed" id="cartButtonHolder"><a href="#0" class="btn_1 gradient full-width" id="theCartBottomButton" onClick="openNav()" >{{ __('Order Summary') }} <span v-if="counter>0">( @{{ counter }} )</span></a></div>
                        @endif
                        <div class='holder-right mb-3'>
                            ...
                            <div class="about-us text-center">
                                ...
                                <div class="social-buttons mb-4">
                                    ...
                                    <div class='item'>
                                        <a href='{{ $facebook }}'>
                                            <svg width="24" fill="#868686" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M12.001 2C6.47 2 2 6.477 2 11.99c0 4.99 3.65 9.126 8.437 9.879v-6.99H7.89v-2.9h2.54V9.76c0-2.51 1.49-3.9 3.776-3.9 1.09 0 2.24.19 2.24.19V8.5h-1.27c-1.24 0-1.63.77-1.63 1.56v1.875h2.77l-.45 2.89h-2.33v6.98c4.78-.75 8.437-4.89 8.437-9.877 0-5.53-4.477-10-9.999-10Z"/></svg>
                                ...
                                @if (strlen($youtube)>2)
                                    <div class='item'>
                                        <a href='{{ $youtube }}'>
                                            <svg width="24" fill="#868686" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M21.593 7.2c-.23-.86-.91-1.54-1.77-1.766 -1.57-.43-7.84-.44-7.84-.44s-6.27-.01-7.831.4c-.84.22-1.54.92-1.766 1.77 -.42 1.56-.42 4.81-.42 4.81s-.01 3.26.4 4.814c.23.85.9 1.53 1.763 1.76 1.58.43 7.83.43 7.83.43s6.26 0 7.83-.41c.85-.23 1.53-.91 1.76-1.77 .41-1.57.41-4.82.41-4.82s.02-3.27-.41-4.84ZM9.996 15l0-6 5.2 3 -5.22 2.99Z"/></svg>
                                        </a>
                                    </div>
                                @endif
                                ...
                                @if (strlen($twitter)>2)
                                    <div class='item'>
                                        <a href='{{ $twitter }}'>
                                            <svg width="24" fill="#868686" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"><path d="M389.2 48h70.6L305.6 224.2 487 464H345L233.7 318.6 106.5 464H35.8l164.9-188.5L26.8 48h145.6l100.5 132.9L389.2 48Zm-24.8 373.8h39.1L151.1 88h-42l255.3 333.8Z"/></svg>
                                        </a>
                                    </div>
                                @endif  
                                ...
                                @if (strlen($website)>2)
                                    <div class='item'>
                                        <a href='{{ $website }}'>
                                            <svg width="24" fill="#868686" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M12 2C6.48 2 2 6.48 2 12c0 5.51 4.48 10 10 10 5.51 0 10-4.49 10-10 0-5.52-4.49-10-10-10Zm7.93 9h-2.77c-.12-2.17-.73-4.3-1.8-6.243 2.43 1.141 4.2 3.47 4.55 6.243Zm-7.41-6.98c1.03 1.36 2.42 3.78 2.62 6.97H9.01C9.14 8.39 10 5.96 11.46 4.01c.17-.01.34-.03.51-.03 .17 0 .35.01.53.02Zm-3.85.7c-.99 1.891-1.56 4.03-1.66 6.27H4.04c.35-2.8 2.14-5.15 4.61-6.28ZM4.05 12.99h2.97c.13 2.37.665 4.47 1.55 6.23 -2.43-1.15-4.183-3.468-4.53-6.23Zm7.38 6.97c-1.41-1.7-2.23-4.08-2.41-6.98h6.11c-.21 2.77-1.12 5.19-2.61 6.97 -.19.01-.37.02-.56.02 -.19 0-.37-.02-.55-.03Zm4.01-.78c.95-1.8 1.53-3.901 1.69-6.21h2.77c-.35 2.73-2.08 5.04-4.47 6.2Z"/></svg>
                                        </a>
                                    </div>
                                @endif  
