    ...
    <div class='box-info bg-white'>
        <div class="box-order">
            <div class='head align-center'>
                <h6>{{ __('Order Summary') }}
                @if ($restorant->minimum != '')
                <br><small>{{ __('Minimum order') }}  @money($restorant->minimum, config('settings.cashier_currency'),config('settings.do_convertion'))</small></h6>
                @endif
            </div>
        </div>
